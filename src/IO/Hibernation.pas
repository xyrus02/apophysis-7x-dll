...
uses RenderingCommon, RenderingInterface, SysUtils, Windows, Forms, Classes,
  Binary, ControlPoint;
...
procedure HibWriteData(const handle: File; header: THibHeader;
  flame: TControlPoint; buckets: TBucketArray;
...
procedure HibReadData(const handle: File; header: THibHeader; var flame: TStringList;
  var buckets: TBucketArray; var colormap: TColorMapArray; callback: TOnProgress);
...
procedure HibWriteData(const handle: File; header: THibHeader; flame: TControlPoint;
  buckets: TBucketArray; colormap: TColorMapArray; callback: TOnProgress);
var
  block: TBlock;
  flametext: string;
...
begin
  ...
  Int64ToBlock(block, 8, 32 * header.Size2D.X * header.Size2D.Y);
  BlockWrite(handle, block, 1);
  ...
  for j := 0 to header.Size2D.Y - 1 do
    for i := 0 to header.Size2D.X - 1 do
      with buckets[j][i] do begin
        DoubleToBlock(block, 0, Red);
        DoubleToBlock(block, 8, Green);
        BlockWrite(handle, block, 1);
        DoubleToBlock(block, 0, Blue);
        DoubleToBlock(block, 8, Count);
        BlockWrite(handle, block, 1);
        p := p + step;
        c := (c + 1) mod 64;
        if (c = 0) then begin
